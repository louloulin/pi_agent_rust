...
/// Minimal persistence sink required by the foreign-session converter.
/// Implementations own storage, indexing, projection and codec details.
pub trait SessionImportSink {
    ...
    fn save(&mut self) -> Result<std::path::PathBuf>;
}

pub trait SessionImportFactory {
    fn sessions_dir(&self) -> std::path::PathBuf;
    fn create(&self, target_dir: std::path::PathBuf) -> Box<dyn SessionImportSink>;
}
...
/// and counted).
pub fn import_claude_with<F: SessionImportFactory>(path: &Path, target_dir: Option<&Path>, factory: &F) -> Result<ImportOutcome> {
    let raw = std::fs::read(path)
        .map_err(|e| Error::tool("import", format!("failed to read {}: {e}", path.display())))?;
    import_bytes(ImportSource::Claude, &raw, path, target_dir, factory)
}
...
/// # Errors
/// Read failures.
pub fn import_codex_with<F: SessionImportFactory>(path: &Path, target_dir: Option<&Path>, factory: &F) -> Result<ImportOutcome> {
    let raw = std::fs::read(path)
        .map_err(|e| Error::tool("import", format!("failed to read {}: {e}", path.display())))?;
    import_bytes(ImportSource::Codex, &raw, path, target_dir, factory)
}
...
fn import_bytes<F: SessionImportFactory>(
    source: ImportSource,
    raw: &[u8],
    original_path: &Path,
    ...
    let mut session = factory.create(target_root);
    session.set_identity(
        &id,
        source.as_str(),
        &format!("foreign-{}", source.as_str()),
        &std::env::current_dir()
    ...
    // Persist through the factory-owned session adapter.
    let actual_path = session
        .save()
...
fn record_unmappable_line(
    session: &mut dyn SessionImportSink,
    line_no: usize,
    note: &str,
    line: &str,
    skipped: &mut usize,
