...
use pi_ai::secret_screener::screen_secrets as screener_screen_secrets;
use serde::Serialize;
...
use pi_ai::provider::{Context, StreamEvent, StreamOptions};
...
pub struct ReflectTool {
    ...
    provider: Option<Arc<dyn pi_ai::provider::Provider>>,
}
...
impl ReflectTool {
    ...
    pub fn with_provider(
        store: Arc<MemoryStore>,
        provider: Arc<dyn pi_ai::provider::Provider>,
    ) -> Self {
        Self {
            store,
    ...
    fn resolve_provider(&self) -> Result<Arc<dyn pi_ai::provider::Provider>> {
...
impl Tool for ReflectTool {
    ...
    async fn execute(
        ...
        let context = Context {
            ...
            messages: std::borrow::Cow::Owned(vec![pi_ai::model::Message::User(
                pi_ai::model::UserMessage {
                    content: pi_ai::model::UserContent::Text(prompt),
                    timestamp: now_ms(),
                },
            )]),
