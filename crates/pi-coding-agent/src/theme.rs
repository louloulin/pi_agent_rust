...
use pi_chord::theme::{ThemeColors, SyntaxColors, UiColors, TerminalBackground};
#[cfg(feature = "tui")]
...
pub use pi_chord::theme::{
    ThemeColors, SyntaxColors, UiColors, TerminalBackground, classify_colorfgbg,
    detect_terminal_background, parse_hex_color, validate_color,
};
...
impl Theme {
    ...
    fn validate(&self) -> Result<()> {
        ...
        validate_color("colors.foreground", &self.colors.foreground)?;
        validate_color("colors.background", &self.colors.background)?;
        validate_color("colors.accent", &self.colors.accent)?;
        validate_color("colors.success", &self.colors.success)?;
        validate_color("colors.warning", &self.colors.warning)?;
        validate_color("colors.error", &self.colors.error)?;
        validate_color("colors.muted", &self.colors.muted)?;

        validate_color("syntax.keyword", &self.syntax.keyword)?;
        validate_color("syntax.string", &self.syntax.string)?;
        validate_color("syntax.number", &self.syntax.number)?;
        validate_color("syntax.comment", &self.syntax.comment)?;
        validate_color("syntax.function", &self.syntax.function)?;

        validate_color("ui.border", &self.ui.border)?;
        validate_color("ui.selection", &self.ui.selection)?;
        validate_color("ui.cursor", &self.ui.cursor)?;
...
mod tests {
    ...
    fn validate_color_valid() {
        assert!(validate_color("test", "#000000").is_ok());
        assert!(validate_color("test", "#ffffff").is_ok());
        assert!(validate_color("test", "#AbCdEf").is_ok());
    }

    #[test]
    fn validate_color_invalid_no_hash() {
        assert!(validate_color("test", "000000").is_err());
    }

    #[test]
    fn validate_color_invalid_too_short() {
        assert!(validate_color("test", "#123").is_err());
    }

    #[test]
    fn validate_color_invalid_chars() {
        assert!(validate_color("test", "#ZZZZZZ").is_err());
    }
