...
pub use pi_ai::model::ThinkingLevel;
pub use pi_ai::model::{
...
pub struct SessionOptions {
    ...
    pub thinking: Option<pi_ai::model::ThinkingLevel>,
...
pub struct RpcCycleModelResult {
    ...
    pub thinking_level: pi_ai::model::ThinkingLevel,
    pub is_scoped: bool,
}
...
pub struct RpcThinkingLevelResult {
    pub level: pi_ai::model::ThinkingLevel,
}
...
impl AgentSessionHandle {
    ...
    pub const fn thinking_level(&self) -> Option<pi_ai::model::ThinkingLevel> {
        self.session.agent.stream_options().thinking_level
    }

    /// Alias for thinking level access, matching the SDK naming style.
    pub const fn thinking(&self) -> Option<pi_ai::model::ThinkingLevel> {
        self.thinking_level()
    }
...
fn stream_event_from_assistant_message_event(
    event: &pi_ai::model::AssistantMessageEvent,
) -> Option<StreamEvent> {
    use pi_ai::model::AssistantMessageEvent as AME;
    match event {
...
mod tests {
    ...
    fn create_agent_session_respects_provider_model_and_clamps_thinking() {
        ...
        let options = SessionOptions {
            ...
            model: Some("gpt-4o".to_string()),
            api_key: Some("dummy-key".to_string()),
            thinking: Some(pi_ai::model::ThinkingLevel::Low),
    ...
    fn create_agent_session_set_thinking_level_clamps_and_dedupes_history() {
        ...
        run_async(handle.set_thinking_level(pi_ai::model::ThinkingLevel::High))
            .expect("set thinking");
        run_async(handle.set_thinking_level(pi_ai::model::ThinkingLevel::High))
    ...
    fn sdk_stream_options_enable_prompt_caching_by_default() {
        ...
        let selection = app::ModelSelection {
            ...
            thinking_level: pi_ai::model::ThinkingLevel::Off,
        ...
        if std::env::var("PI_CACHE_RETENTION").is_err() {
            assert_eq!(
                options.cache_retention,
                pi_ai::provider::CacheRetention::Short
            );
        }
    }
