...
use super::*;
use pi_ai::provider_metadata::{canonical_provider_id, provider_ids_match};
...
impl InteractiveExtensionHostActions {
    ...
    ) -> pi_error::Result<OwnedMutexGuard<()>> {
    ...
    async fn append_idle_message(&self, message: ModelMessage) -> pi_error::Result<String> {
        ...
        let mut agent_guard = self
            ...
            .lock(&cx)
            .await
            .map_err(|e| pi_error::Error::session(e.to_string()))?;
        let Ok(mut session_guard) = self.session.try_lock() else {
            return Err(pi_error::Error::session(
    ...
    fn current_session_id(&self) -> pi_error::Result<String> {
...
async fn acquire_session_action_admission(
    ...
    let origin = origin.ok_or_else(|| {
        pi_error::Error::session("extension Session action is missing trusted task provenance")
    })?;
...
impl ExtensionSession for InteractiveExtensionSession {
    ...
    async fn set_name(
        ...
        let mut guard = OwnedMutexGuard::lock(Arc::clone(&self.session), &cx)
            .await
            .map_err(|err| pi_error::Error::session(format!("session lock failed: {err}")))?;
        guard.set_name(&name);
    ...
    async fn set_thinking_level(
        ...
        let mut guard = OwnedMutexGuard::lock(Arc::clone(&self.session), &cx)
            .await
            .map_err(|err| pi_error::Error::session(format!("session lock failed: {err}")))?;
        let effective_level = level.parse::<pi_ai::model::ThinkingLevel>().map_or_else(
            |_| level.clone(),
    ...
    fn branch_without_overrides_does_not_inherit_stale_header_state() {
        ...
        runtime.block_on(async {
            ...
            let branch_a_tip = session_state.append_message(SessionMessage::User {
                content: pi_ai::model::UserContent::Text("branch-a".to_string()),
                timestamp: Some(0),
            });
