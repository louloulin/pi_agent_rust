...
impl PiApp {
    fn normalize_model_key(entry: &ModelEntry) -> (String, String) {
        let canonical_provider =
            pi_ai::provider_metadata::canonical_provider_id(entry.model.provider.as_str())
    ...
    fn available_models_with_credentials(&self) -> Vec<ModelEntry> {
        ...
        for entry in &self.available_models {
            ...
            let canonical = pi_ai::provider_metadata::canonical_provider_id(provider)
                .unwrap_or(provider)
                .to_ascii_lowercase();
...
mod tests {
    ...
    use pi_ai::model::{StreamEvent, Usage};
    ...
    fn apply_model_selection_clamps_thinking_level_for_non_reasoning_targets() {
        ...
        {
            let mut guard = app.agent.try_lock().expect("agent lock");
            guard.stream_options_mut().thinking_level = Some(pi_ai::model::ThinkingLevel::High);
        }
        {
            let mut guard = app.session.try_lock().expect("session lock");
            guard.header.thinking_level = Some(pi_ai::model::ThinkingLevel::High.to_string());
        }
