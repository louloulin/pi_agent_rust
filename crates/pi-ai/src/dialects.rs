...
//! ## Stage 2 extraction
//!
//! This crate is a pure leaf: it depends only on `serde` and `serde_json`
//! and never touches the legacy meta-crate's `error` / `tools` / `model`
//! machinery. The original implementation lived at
//! `crates/pi/src/dialects.rs`; the legacy module now re-exports these
//! items so existing call sites (`use crate::dialects::*`,
//! `pi::dialects::Dialect`, etc.) keep working unchanged.
