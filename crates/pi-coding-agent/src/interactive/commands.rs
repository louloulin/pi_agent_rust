...
pub(super) fn normalize_auth_provider_input(raw: &str) -> String {
    ...
    pi_ai::provider_metadata::canonical_provider_id(&provider)
...
fn extension_oauth_config_for_provider(
    ...
    registered_extension_bindings
        ...
        .find_map(|binding| {
            ...
            let canonical = pi_ai::provider_metadata::canonical_provider_id(registered_provider)
        ...
        .or_else(|| {
            available_models.iter().find_map(|entry| {
                ...
                let canonical = pi_ai::provider_metadata::canonical_provider_id(model_provider)
...
) -> pi_error::Result<Vec<ExtensionProviderBinding>> {
...
pub(super) fn format_login_provider_listing(
    ...
    let mut api_key_rows: Vec<(String, String, String)> =
        pi_ai::provider_metadata::PROVIDER_METADATA
            .iter()
...
pub(super) fn should_show_startup_oauth_hint(auth: &crate::auth::AuthStorage) -> bool {
    let has_any_credential = pi_ai::provider_metadata::PROVIDER_METADATA
        .iter()
...
pub fn model_entry_matches(left: &ModelEntry, right: &ModelEntry) -> bool {
    let left_provider = pi_ai::provider_metadata::canonical_provider_id(&left.model.provider)
        .unwrap_or(&left.model.provider);
    let right_provider = pi_ai::provider_metadata::canonical_provider_id(&right.model.provider)
...
struct SessionThinkingSyncPlan {
    effective: pi_ai::model::ThinkingLevel,
...
fn plan_session_thinking_sync(
    ...
    current_thinking: pi_ai::model::ThinkingLevel,
...
impl PiApp {
    ...
    pub(super) fn switch_active_model(
        &mut self,
        next: &ModelEntry,
        provider_impl: std::sync::Arc<dyn pi_ai::provider::Provider>,
        ...
        agent_guard.set_keyword_max_thinking_level(
            next.clamp_thinking_level(pi_ai::model::ThinkingLevel::Max),
        );
    ...
    pub(super) fn sync_runtime_selection_from_session_header(&mut self) -> Result<(), String> {
        ...
        agent_guard.set_keyword_max_thinking_level(
            target_entry.clamp_thinking_level(pi_ai::model::ThinkingLevel::Max),
        );
            ...
            let credential = match kind {
                ...
                PendingLoginKind::OAuth => {
                    if provider == "anthropic" {
                        ...
                        Err(pi_error::Error::auth(format!(
    ...
    pub(super) fn handle_slash_command(&mut self, cmd: SlashCommand, args: &str) -> Option<Cmd> {
        ...
        match cmd {
            ...
            SlashCommand::Changelog => {
                let content = pi_ai::embedded_assets::changelog().to_string();
                self.messages.push(ConversationMessage {
                    role: MessageRole::System,
                    content,
                    thinking: None,
...
result in account suspension/ban. Prefer using an Anthropic API key (ANTHROPIC_API_KEY) instead.\n",
    ...
    pub(super) fn handle_slash_rules(&mut self, args: &str) -> Option<Cmd> {
        ...
        let mut store = pi_ai::stream_rules::StreamRuleStore::load_for_project(&project_root);
    ...
    pub(super) fn handle_slash_tan(&mut self, args: &str) -> Option<Cmd> {
        ...
        let tool = crate::subagents::SubagentTool::new(&self.cwd)
            .with_role_model_spec(crate::app::subagent_role_spec(&self.config));
        let runtime = self.runtime_handle.clone();
        let event_tx = self.event_tx.clone();
        let task_cx = Cx::current().unwrap_or_else(Cx::for_request);
    ...
    pub(super) fn handle_slash_crash(&mut self, args: &str) -> Option<Cmd> {
        ...
        match args.trim() {
            "" => {
                let bundles = crate::crash::list_bundles(&agent_dir);
            ...
            "show" => {
                let report = crate::crash::show_latest(&agent_dir)
            ...
            "delete" => {
                let removed = crate::crash::delete_all(&agent_dir);
    ...
    pub(super) fn handle_slash_omfg(&mut self, args: &str) -> Option<Cmd> {
        ...
        let project_root = self.cwd.clone();
        match pi_ai::stream_rules::GrievancesLedger::record_complaint(&project_root, args, None) {
            Ok(g) => {
                let candidate = pi_ai::stream_rules::GrievancesLedger::forge_candidate_rule(&g);
                let mut store =
                    pi_ai::stream_rules::StreamRuleStore::load_for_project(&project_root);
    ...
    fn switch_to_role_spec(&mut self, spec: &str, source: &str) {
        let Some((provider, model_id)) = pi_ai::provider_metadata::split_provider_model_spec(spec)
        else {
            return;
        };
        let entry = self
...
mod tests {
    ...
    use pi_ai::model::{Message as ModelMessage, StreamEvent, Usage, UserContent, UserMessage};
    ...
    impl Provider for DummyProvider {
        ...
        async fn stream(
            ...
            Pin<Box<dyn futures::Stream<Item = pi_error::Result<StreamEvent>> + Send>>,
        > {
    ...
    fn completed_tan_event_keeps_card_and_follow_up_bound_to_the_origin_session() {
        ...
        let completion = crate::subagents::TanCompletion {
    ...
    fn plan_session_thinking_sync_repairs_missing_header_when_model_clamps_runtime_level() {
        ...
        let plan =
            super::plan_session_thinking_sync(None, pi_ai::model::ThinkingLevel::High, &target);

        assert_eq!(plan.effective, pi_ai::model::ThinkingLevel::Off);
