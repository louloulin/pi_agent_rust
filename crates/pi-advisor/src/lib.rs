        ...
        match message {
            Message::Assistant(assistant) => {
                for block in &assistant.content {
                    match block {
                        pi_model::ContentBlock::ToolCall(call) => {
                        ...
                        pi_model::ContentBlock::Text(text) => {
                            digest.final_text =
                ...
                if result.is_error && digest.tool_errors.len() < MAX_DIGEST_ERRORS =>
            {
                let text = result
                    ...
                    .filter_map(|block| match block {
                        pi_model::ContentBlock::Text(t) => Some(t.text.clone()),
                        _ => None,
                    })
                    .collect::<Vec<_>>()
                    .join(" ");
...
impl AdvisorRuntime {
    ...
    async fn call_advisor(&self, digest: &TurnDigest) -> Result<String> {
        use futures::StreamExt;
        let context = pi_provider::Context {
            system_prompt: Some(REVIEW_SYSTEM_PROMPT.to_string().into()),
            messages: vec![pi_model::Message::User(pi_model::UserMessage {
                content: pi_model::UserContent::Text(digest_prompt(digest)),
                timestamp: chrono::Utc::now().timestamp_millis(),
            })]
            .into(),
            tools: Vec::new().into(),
        };
        let options = pi_provider::StreamOptions {
            max_tokens: Some(512),
        ...
        while let Some(event) = stream.next().await {
            match event {
                Ok(pi_model::StreamEvent::TextDelta { delta, .. }) => text.push_str(&delta),
                Ok(pi_model::StreamEvent::Done { .. }) => break,
                Ok(_) => {}
                Err(err) => return Err(err),
            }
        }
        if text.trim().is_empty() {
            return Err(Error::api("advisor returned empty reply"));
        }
        Ok(text)
    }
}
...
mod tests {
    ...
    fn digest_collects_files_commands_errors_and_text() {
        let messages = vec![
            Message::User(pi_model::UserMessage {
                content: pi_model::UserContent::Text("fix it".to_string()),
                timestamp: 0,
            }),
            Message::Assistant(Arc::new(pi_model::AssistantMessage {
                content: vec![
                    pi_model::ContentBlock::ToolCall(pi_model::ToolCall {
                        id: "1".to_string(),
                        name: "edit".to_string(),
                    ...
                    pi_model::ContentBlock::Text(pi_model::TextContent::new("done")),
                ],
                api: "x".to_string(),
                provider: "y".to_string(),
                model: "z".to_string(),
                usage: pi_model::Usage::default(),
                stop_reason: pi_model::StopReason::Stop,
                stop_details: None,
                error_message: None,
                timestamp: 0,
            })),
            Message::ToolResult(Arc::new(pi_model::ToolResultMessage {
                tool_call_id: "2".to_string(),
                tool_name: "bash".to_string(),
                content: vec![pi_model::ContentBlock::Text(pi_model::TextContent::new(
                    "permission denied",
                ))],
                is_error: true,
                details: None,
    ...
    fn digest_flags_trivial_turns() {
        let messages = vec![Message::User(pi_model::UserMessage {
            content: pi_model::UserContent::Text("hi".to_string()),
            timestamp: 0,
        })];
        let digest = build_digest(&messages);
