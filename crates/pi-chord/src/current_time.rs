...
mod tests {
    ...
    #[test]
    fn pinned_snapshot_formats_all_fields() {
        let utc = Utc.with_ymd_and_hms(2026, 9, 1, 23, 15, 42).unwrap();
        let offset = FixedOffset::west_opt(4 * 3600).unwrap();
        let snap = TimeSnapshot::from_instants(utc, utc.with_timezone(&offset), Some("America/New_York".into()));
        ...
        assert!(snap.details().get("unixSeconds").is_some());
    }
}
