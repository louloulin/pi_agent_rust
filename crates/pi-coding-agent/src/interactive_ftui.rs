...
pub enum UiCommand {
    ...
    SetThinking(Option<pi_ai::model::ThinkingLevel>),
    ...
    fn handle_agent(&mut self, msg: PiMsg) -> Cmd<PiFtuiMsg> {
        ...
        match msg {
            ...
            PiMsg::TerminalTitle(title) => {
                ...
                let sequence = pi_ai::delight::format_terminal_title(&title);
    ...
    fn route_slash_command_tail(&mut self, clean: &str) -> bool {
        ...
        match cmd_name.to_ascii_lowercase().as_str() {
            ...
            "/thinking" | "/think" | "/t" => {
                ...
                match value.parse::<pi_ai::model::ThinkingLevel>() {
...
fn tool_output_preview(result: &crate::tools::ToolOutput) -> Option<String> {
    ...
    for block in &result.content {
        if let pi_ai::model::ContentBlock::Text(t) = block {
            if !text.is_empty() {
                text.push('\n');
            }
...
pub fn agent_event_to_pi_msgs(event: &crate::agent::AgentEvent) -> Vec<PiMsg> {
    ...
    use pi_ai::model::AssistantMessageEvent as A;
    ...
    match event {
        ...
        E::AgentEnd {
            ...
            let last_assistant = messages.iter().rev().find_map(|message| match message {
                pi_ai::model::Message::Assistant(assistant) => Some(assistant),
                _ => None,
            });
            let stop_reason =
                last_assistant.map_or(pi_ai::model::StopReason::Stop, |a| a.stop_reason);
            ...
            let error_message = error.as_ref().map(|raw| {
                if stop_reason == pi_ai::model::StopReason::Error {
                    pi_error::ProviderErrorSummary::from_error_text(
...
impl crate::sdk::ExtensionUiHandler for FtuiExtensionUiHandler {
    ...
    ) -> pi_error::Result<Option<ExtensionUiResponse>> {
        let cx = crate::agent_cx::AgentCx::for_current_or_request();
...
async fn run_prompt_turn(
    ...
    *turn_abort
    ...
    match result {
        ...
        Err(err @ pi_error::Error::Provider { .. }) => {
            let raw = err.to_string();
            let headline =
                pi_error::ProviderErrorSummary::from_error_text(None, &raw).headline();
            let _ = agent_tx.send(PiMsg::AgentError(headline));
        }
        Err(err) => {
            let _ = agent_tx.send(PiMsg::AgentError(err.to_string()));
        }
        Ok(message) if message.stop_reason == pi_ai::model::StopReason::Error => {
            let raw = message.error_message.as_deref().unwrap_or("Request failed");
            let headline =
                pi_error::ProviderErrorSummary::from_error_text(Some(&message.provider), raw)
                    .headline();
            let _ = agent_tx.send(PiMsg::AgentError(headline));
        }
        Ok(_) => {}
...
async fn run_set_thinking_command(
    handle: &mut crate::sdk::AgentSessionHandle,
    level: Option<pi_ai::model::ThinkingLevel>,
    agent_tx: &Sender<PiMsg>,
) {
    let msg = match level {
...
fn run_crash_command(action: &str, agent_tx: &Sender<PiMsg>) {
    ...
    let msg = match action {
        ...
        "show" => crate::crash::show_latest(&agent_dir).map_or_else(
            || PiMsg::System(String::from("No crash bundles recorded.")),
            PiMsg::System,
        ),
        "delete" => {
...
mod tests {
    ...
    fn agent_event_translation_covers_lifecycle_stream_and_tools() {
        use crate::agent::AgentEvent as E;
        use pi_ai::model::{AssistantMessage, AssistantMessageEvent as A, Message, Usage};
        use std::sync::Arc;
    ...
    fn session_slash_commands_route_to_driver() {
        ...
        assert_eq!(
            submit_rx.try_recv().expect("routed"),
            UiCommand::SetThinking(Some(pi_ai::model::ThinkingLevel::Medium))
        );
        ...
        assert_eq!(
            submit_rx.try_recv().expect("routed"),
            UiCommand::SetThinking(Some(pi_ai::model::ThinkingLevel::High))
        );
    ...
    fn agent_end_provider_error_translates_to_structured_card() {
        use crate::agent::AgentEvent as E;
        use pi_ai::model::{AssistantMessage, Message};
        use std::sync::Arc;
...
{\"error\":{\"code\":\"service_unavailable_error\",\"message\":\"Server Overloaded\"}}";
        ...
        let summary = pi_error::ProviderErrorSummary::from_error_text(Some("deepseek"), raw);
        assert!(
