...
use pi_ai::provider::{Context, Provider, StreamOptions, ToolDef};
...
pub(crate) fn append_dialect_repair_telemetry(
    session: &mut Session,
    repairs: &[pi_ai::dialects::RepairEntry],
) {
...
fn record_global_latency(counter: &pi_telemetry::session_metrics::TimingCounter, duration: Duration) {
    if pi_telemetry::session_metrics::global().enabled() {
...
fn record_provider_streaming_latency(latency: &SharedTurnLatencyAccumulator, duration: Duration) {
    ...
    let metrics = pi_telemetry::session_metrics::global();
...
pub struct AgentConfig {
    ...
    pub keyword_settings: Option<pi_ai::magic_keywords::KeywordSettings>,
...
impl AgentEvent {
    ...
    pub fn to_json_stream_line(&self) -> serde_json::Result<String> {
        ...
        struct DeltaOnlyMessageUpdate<'a> {
            ...
            assistant_message_event: pi_ai::model::AssistantMessageEventDelta<'a>,
        }
...
pub enum AgentEvent {
    ...
    ProviderError {
        ...
        model: String,
        #[serde(flatten)]
        summary: pi_error::ProviderErrorSummary,
        message: String,
    },
...
pub struct Agent {
    ...
    repair_ledger: Arc<StdMutex<pi_ai::dialects::RepairLedger>>,
    ...
    tool_call_dialect: pi_ai::dialects::Dialect,
    ...
    keyword_ledger: Vec<pi_ai::magic_keywords::KeywordActivation>,
    ...
    retry_keyword_activations: Vec<pi_ai::magic_keywords::KeywordActivation>,
    ...
    keyword_max_thinking_level: pi_ai::model::ThinkingLevel,
...
impl Agent {
    ...
    pub fn with_shared_tools(
        ...
        let keyword_max_thinking_level = config
            .stream_options
            .thinking_level
            .unwrap_or(pi_ai::model::ThinkingLevel::Off);
        ...
        Self {
            ...
            repair_ledger: Arc::new(StdMutex::new(pi_ai::dialects::RepairLedger::default())),
            dialect_repair_sequence: AtomicU64::new(0),
            tool_call_dialect: pi_ai::dialects::Dialect::Native,
    ...
    fn maybe_repair_dialect_tool_calls(&self, msg: AssistantMessage) -> AssistantMessage {
        use pi_ai::dialects::{Dialect, extract_text_tool_calls, strip_candidates};
    ...
    pub fn drain_repair_ledger(&self) -> Result<Vec<pi_ai::dialects::RepairEntry>> {
        self.repair_ledger
            .lock()
    ...
    pub fn drain_keyword_ledger(&mut self) -> Vec<pi_ai::magic_keywords::KeywordActivation> {
    ...
    pub fn set_provider(&mut self, provider: Arc<dyn Provider>) {
        ...
        self.keyword_max_thinking_level = pi_ai::model::ThinkingLevel::Off;
        self.tool_call_dialect = pi_ai::dialects::Dialect::Native;
    }

    /// Set the model-clamped target used when a turn contains `ultrathink`.
    pub const fn set_keyword_max_thinking_level(&mut self, level: pi_ai::model::ThinkingLevel) {
    ...
    pub const fn set_tool_call_dialect(&mut self, dialect: pi_ai::dialects::Dialect) {
    ...
    #[must_use]
    pub const fn tool_call_dialect(&self) -> pi_ai::dialects::Dialect {
    ...
    fn build_provider_error_event(&self, session_id: &Arc<str>, message: &str) -> AgentEvent {
        ...
        AgentEvent::ProviderError {
            ...
            summary: pi_error::ProviderErrorSummary::from_error_text(Some(&provider), message),
            provider,
            model: self.provider.model_id().to_string(),
    ...
    fn apply_magic_keyword_effects(&mut self, hits: &[pi_ai::magic_keywords::KeywordActivation]) {
        ...
        let directives =
            pi_ai::magic_keywords::directives_for(hits, self.config.keyword_settings.as_ref());
    ...
    fn apply_magic_keywords_for_text(
        ...
        let hits: Vec<_> =
            pi_ai::magic_keywords::detect(scan_text, self.config.keyword_settings.as_ref())
                .into_iter()
            ...
            while has_more_tool_calls || !pending_messages.is_empty() {
                    ...
                    && run_started.elapsed() >= cap
                {
                    ...
                    let assistant = Arc::new(AssistantMessage {
                        content: vec![pi_ai::model::ContentBlock::Text(TextContent::new(format!(
                ...
                for delivery in std::mem::take(&mut pending_messages) {
                    ...
                    if let Message::User(user) = &message
                        && let pi_ai::model::UserContent::Text(text) = &user.content
                ...
                if execute_local_tools {
                    ...
                    if iterations > self.config.max_tool_iterations {
                        ...
                        stop_message
                            .content
                            .retain(|b| !matches!(b, pi_ai::model::ContentBlock::ToolCall(_)));
    ...
    ) -> pi_ai::provider::BeforeProviderRequestHook {
        pi_ai::provider::BeforeProviderRequestHook::new(move |event| {
...
struct PreparedModelSelection {
    ...
    provider_id: String,
    model_id: String,
    thinking_level: pi_ai::model::ThinkingLevel,
}
    ...
    fn direct_authored_block_message_preserves_all_text_for_scanning() {
        let queued = QueuedAgentMessage::from_authored_message(Message::User(UserMessage {
            content: UserContent::Blocks(vec![
                ...
                ContentBlock::Image(pi_ai::model::ImageContent {
...
mod extensions_integration_tests {
    ...
    impl Provider for PiAiCaptureProvider {
        ...
        async fn stream(
            ...
            std::pin::Pin<
                Box<dyn futures::Stream<Item = pi_error::Result<StreamEvent>> + Send>,
            >,
        > {
            self.calls
                .lock()
            ...
            let nudges = agent
                ...
                .filter(|message| {
                    matches!(message, Message::User(user)
                        if matches!(&user.content, pi_ai::model::UserContent::Text(text)
...
mod abort_tests {
    ...
    impl Stream for StartThenPending {
        type Item = pi_error::Result<StreamEvent>;
...
impl AgentSession {
    ...
    fn prepare_model_selection(
        ...
        provider_id: &str,
        model_id: &str,
        requested_thinking: pi_ai::model::ThinkingLevel,
    ...
    pub async fn set_provider_model(&mut self, provider_id: &str, model_id: &str) -> Result<()> {
        ...
        let previous_thinking = candidate
            ...
            .as_deref()
            .and_then(|value| value.parse::<pi_ai::model::ThinkingLevel>().ok());
        if previous_model
            .as_ref()
    ...
    pub async fn set_thinking_level(&mut self, level: pi_ai::model::ThinkingLevel) -> Result<()> {
        self.ensure_provider_reentry_allowed()?;
        let cx = crate::agent_cx::AgentCx::for_request();
        let save_enabled = self.save_enabled;
        let session_store = Arc::clone(&self.session);
    ...
    pub(crate) fn clamp_thinking_level_for_model(
        ...
        provider_id: &str,
        model_id: &str,
        level: pi_ai::model::ThinkingLevel,
    ) -> pi_ai::model::ThinkingLevel {
        self.model_registry
            .as_ref()
            .and_then(|registry| registry.find(provider_id, model_id))
    ...
    pub(crate) async fn sync_runtime_selection_from_session_header(&mut self) -> Result<()> {
        ...
        let parsed_session_thinking = session_thinking.as_deref().and_then(|raw| {
            raw.parse::<pi_ai::model::ThinkingLevel>().map_or_else(
                |_| {
    ...
    async fn maybe_advise_turn(&mut self) {
        ...
        let verdict = match outcome {
            ...
            crate::advisor::AdvisorOutcome::Failed => {
                ...
                if let Some(notice) = runtime.disabled_notice.clone() {
                    let msg = pi_ai::model::Message::User(pi_ai::model::UserMessage {
                        content: pi_ai::model::UserContent::Text(format!(
        ...
        let message = pi_ai::model::Message::User(pi_ai::model::UserMessage {
            content: pi_ai::model::UserContent::Text(injection.clone()),
            timestamp: chrono::Utc::now().timestamp_millis(),
        });
    ...
    async fn persist_turn_artifacts(
        ...
        };
        {
            ...
            pi_ai::magic_keywords::append_session_telemetry(&mut *session, &activations);
...
mod tests {
    ...
    impl Provider for PricedUsageProvider {
        ...
        fn model_cost(&self) -> Option<pi_ai::provider::ModelCost> {
            Some(pi_ai::provider::ModelCost {
                input: 2.0,
                output: 10.0,
                cache_read: 0.2,
                cache_write: 2.5,
    ...
    fn run_priced_turn(provider_reported_total: Option<f64>) -> pi_ai::model::Usage {
        let runtime = RuntimeBuilder::current_thread()
            .build()
            .expect("runtime build");
        runtime.block_on(async {
    ...
    struct CapturedProviderContext {
        ...
        messages: Vec<Message>,
        thinking_level: Option<pi_ai::model::ThinkingLevel>,
    }
    ...
    fn queued_steering_and_follow_up_messages_apply_magic_keywords_on_delivery() {
        ...
        runtime.block_on(async {
            ...
            let mut agent = Agent::new(
                ...
                AgentConfig {
                    stream_options: StreamOptions {
                        thinking_level: Some(pi_ai::model::ThinkingLevel::Low),
                        ..StreamOptions::default()
                    },
                    ..AgentConfig::default()
                },
            );
            agent.set_keyword_max_thinking_level(pi_ai::model::ThinkingLevel::High);
    ...
    fn continuation_follow_up_first_precedes_provider_and_later_steering() {
        ...
        runtime.block_on(async {
            ...
            assert_eq!(
                recorded_calls[0].thinking_level,
                Some(pi_ai::model::ThinkingLevel::Low),
    ...
    fn prepared_model_selection_updates_stream_credentials_and_headers() {
        ...
        let prepared = agent_session
            .prepare_model_selection("openai", "gpt-4o", pi_ai::model::ThinkingLevel::Off)
    ...
    fn prepared_model_selection_treats_blank_model_key_as_missing_credential() {
        ...
        agent_session.set_model_registry(registry);
        let err = agent_session
            .prepare_model_selection("acme", "blank-model", pi_ai::model::ThinkingLevel::Off)
            .err()
    ...
    fn set_thinking_level_quarantines_failed_persistence_without_live_mutation() {
        ...
        runtime.block_on(async {
            ...
            let err = agent_session
                .set_thinking_level(pi_ai::model::ThinkingLevel::High)
                .await
    ...
    fn bare_provider_replacement_resets_catalog_controls_fail_closed() {
        ...
        agent.set_keyword_max_thinking_level(pi_ai::model::ThinkingLevel::High);
        agent.set_tool_call_dialect(pi_ai::dialects::Dialect::Xmlish);
    ...
    struct TextCallProvider {
        ...
        expected_retry_thinking: Option<pi_ai::model::ThinkingLevel>,
    }
    ...
    fn dialect_repair_continues_turn_and_executes_tool() {
        ...
        runtime.block_on(async {
            ...
            let texts: Vec<String> = agent_session
                ...
                .iter()
                .flat_map(|m| match m {
                    pi_ai::model::Message::Assistant(msg) => msg.content.clone(),
                    pi_ai::model::Message::User(u) => match &u.content {
                        pi_ai::model::UserContent::Text(t) => {
                            vec![ContentBlock::Text(TextContent::new(t))]
                        }
                        pi_ai::model::UserContent::Blocks(blocks) => blocks.clone(),
                    },
                    pi_ai::model::Message::ToolResult(r) => r.content.clone(),
                    pi_ai::model::Message::Custom(_) => Vec::new(),
                })
    ...
    fn failed_keyword_repair_turn_keeps_audits_and_reverts_incomplete_tail() {
        ...
        runtime.block_on(async {
            ...
            let provider = Arc::new(TextCallProvider {
                ...
                expected_retry_thinking: Some(pi_ai::model::ThinkingLevel::High),
            });
            let tools = ToolRegistry::new(&["read"], temp.path(), None);
            let mut agent = Agent::new(
        ...
        runtime.block_on(async {
            ...
            let steered = agent_session
                ...
                .filter_map(|m| match m {
                    pi_ai::model::Message::User(u) => match &u.content {
                        pi_ai::model::UserContent::Text(t) => Some(t.clone()),
                        pi_ai::model::UserContent::Blocks(_) => None,
                    },
                    _ => None,
                })
