 ...
 *
 * Usage:
 * - `pi -e ./sandbox` - sandbox disabled by default
 * - `pi -e ./sandbox --sandbox` - enable sandboxing
...
export default function (pi: ExtensionAPI) {
	...
	pi.on("session_start", async (_event, ctx) => {
		const sandboxFlag = pi.getFlag("sandbox") as boolean;
		const config = loadConfig(ctx.cwd);

		if (!sandboxFlag && !config.enabled) {
