...
use pi_error::Error;
use pi_ai::provider::{Api, InputType, Model, ModelCost};
...
impl ModelEntry {
    ...
    pub fn tool_call_dialect(&self) -> pi_ai::dialects::Dialect {
        self.compat
            .as_ref()
    ...
    pub fn available_thinking_levels(&self) -> Vec<pi_ai::model::ThinkingLevel> {
    ...
    pub fn clamp_thinking_level(
        &self,
        thinking: pi_ai::model::ThinkingLevel,
    ) -> pi_ai::model::ThinkingLevel {
        if !self.model.reasoning {
            return pi_ai::model::ThinkingLevel::Off;
        }
        let mut thinking = thinking;
        if thinking == pi_ai::model::ThinkingLevel::Max && !self.supports_max() {
            thinking = if self.supports_xhigh() {
                pi_ai::model::ThinkingLevel::XHigh
            } else {
                pi_ai::model::ThinkingLevel::High
            };
        }
        if thinking == pi_ai::model::ThinkingLevel::XHigh && !self.supports_xhigh() {
            return pi_ai::model::ThinkingLevel::High;
        }
        thinking
    }
}
...
pub struct ModelOverrideConfig {
    ...
    pub headers: Option<HashMap<String, String>>,
    pub compat: Option<CompatConfig>,
    pub dialect: Option<pi_ai::dialects::Dialect>,
...
pub struct CompatConfig {
    ...
    pub tool_call_dialect: Option<pi_ai::dialects::Dialect>,
...
fn legacy_generated_models_cache_path() -> Option<PathBuf> {
    let checksum = pi_ai::embedded_assets::legacy_models_generated_ts_crc32c();
...
fn parse_legacy_generated_models() -> Vec<LegacyGeneratedModel> {
    ...
    let source = pi_ai::embedded_assets::legacy_models_generated_ts();
...
fn parse_upstream_provider_model_ids() -> HashMap<String, Vec<String>> {
    let parsed: HashMap<String, Vec<String>> =
        match serde_json::from_str(&pi_ai::embedded_assets::provider_upstream_model_ids_json()) {
            Ok(value) => value,
            Err(err) => {
...
pub fn model_catalog_cache_fingerprint() -> u64 {
    *MODEL_CATALOG_CACHE_FINGERPRINT.get_or_init(|| {
        let legacy = u64::from(pi_ai::embedded_assets::legacy_models_generated_ts_crc32c());
        let upstream = u64::from(pi_ai::embedded_assets::provider_upstream_model_ids_json_crc32c());
...
pub fn model_requires_configured_credential(entry: &ModelEntry) -> bool {
    ...
    entry.auth_header
        || pi_ai::provider_metadata::provider_metadata(provider)
...
mod tests {
    ...
    fn parse_legacy_generated_models_extracts_known_legacy_only_providers() {
        ...
        if pi_ai::embedded_assets::legacy_models_generated_ts()
    ...
    fn deepseek_reasoning_xhigh_survives_clamp_and_serializes_as_max() {
        use pi_ai::model::ThinkingLevel;
        use pi_ai::provider::{Context, StreamOptions};
        ...
        let context = Context {
            ...
            messages: vec![pi_ai::model::Message::User(pi_ai::model::UserMessage {
                content: pi_ai::model::UserContent::Text("solve it".to_string()),
                timestamp: 0,
            })]
            .into(),
            tools: Vec::<pi_ai::provider::ToolDef>::new().into(),
        };
    ...
    fn model_level_dialect_is_explicit_and_defaults_native() {
        ...
        assert_eq!(opt_in.tool_call_dialect(), pi_ai::dialects::Dialect::Xmlish);
        assert_eq!(
    ...
    fn fetched_provider_bound_covers_supported_provider_inventory() {
        assert!(
            pi_ai::provider_metadata::PROVIDER_METADATA.len() <= MAX_FETCHED_PROVIDERS,
