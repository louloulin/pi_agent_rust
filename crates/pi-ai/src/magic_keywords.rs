...
/// Sink for magic-keyword session telemetry.
///
/// The leaf crate owns the data shape and recording semantics; the legacy
/// `pi` crate implements this trait for `session::Session` so that
/// `append_session_telemetry` can be reused by both the agent loop and the
/// interactive TUI without dragging the session module across the leaf
/// boundary.
pub trait MagicKeywordSink {
    /// Record one activation as a custom entry on the underlying session.
...
pub fn append_session_telemetry<S: MagicKeywordSink>(
    sink: &mut S,
    activations: &[KeywordActivation],
) {
    for activation in activations {
        sink.append_keyword_entry(
            KEYWORD_TELEMETRY_SCHEMA_V1,
            activation.word.clone(),
            activation.action.clone(),
        );
    }
}
