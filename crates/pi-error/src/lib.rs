...
/// Local stand-ins for the few provider-metadata helpers used by the
/// hint builder. They intentionally do NOT depend on `pi-ai` to avoid
/// a cycle (`pi-ai` already depends on `pi-error` for `Error` / `Result`).
/// Returning `None` / `&[]` keeps the hint path functional — the
/// generic "Check API key configuration" message is still produced.
fn canonical_provider_id(provider_id: &str) -> Option<&'static str> {
    // Stable, hard-coded canonical ids for the providers whose hint text
    // branches on canonical id (`anthropic`, `github-copilot`). For all
    // other providers the caller's `unwrap_or(provider)` branch fires.
    match provider_id {
...
fn provider_auth_env_keys(_provider_id: &str) -> &'static [&'static str] {
    // Intentionally empty — the hint path falls back to the generic
    // message when the env-key list is empty. The full table lives
    // in `pi_ai::provider_metadata`; rebuilding it here would create a
    // cycle and risk drift.
    &[]
}
