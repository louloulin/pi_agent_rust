...
fn append_turn_artifacts(
    ...
    keyword_activations: &[pi_ai::magic_keywords::KeywordActivation],
) {
    ...
    pi_ai::magic_keywords::append_session_telemetry(session, keyword_activations);
...
) -> pi_error::Result<InputEventOutcome> {
...
async fn generate_session_title(
    ...
    use pi_ai::model::{Message, StreamEvent};
    ...
    let error_message = last.as_ref().and_then(|msg| {
        msg.error_message.as_ref().map(|raw| {
            if stop_reason == StopReason::Error {
                pi_error::ProviderErrorSummary::from_error_text(Some(&msg.provider), raw)
...
After approving access in the browser, press Enter in Pi to complete login."
    ...
    fn submit_continue(&mut self) -> Option<Cmd> {
        ...
        runtime_handle.spawn(async move {
            ...
            let new_messages: Vec<pi_ai::model::Message> =
            ...
            {
                let formatted = pi_ai::error_hints::format_error_with_hints(&err);
                let _ = crate::interactive::enqueue_pi_event(
                    &event_tx,
                    &Cx::for_request(),
...
mod stream_delta_batcher_tests {
    ...
    use pi_ai::model::{AssistantMessage, StreamEvent, Usage};
    use pi_ai::provider::{Context, InputType, Model, ModelCost, Provider, StreamOptions};
...
{\"error\":{\"code\":\"service_unavailable_error\",\"message\":\"Server Overloaded\"}}";
    ...
    fn agent_done_error_after_partial_text_still_pushes_card() {
        ...
        let card = pi_error::ProviderErrorSummary::from_error_text(
            Some("deepseek"),
            OVERLOADED_503_BODY,
        )
        ...
        let (session_id, current_leaf_id, target_leaf_id) = runtime().block_on(async {
            ...
            let current_leaf_id =
                session_guard.append_message(crate::session::SessionMessage::User {
                    content: pi_ai::model::UserContent::Text("current".to_string()),
                    timestamp: Some(0),
                });
            ...
            let target_leaf_id =
                session_guard.append_message(crate::session::SessionMessage::User {
                    content: pi_ai::model::UserContent::Text("target".to_string()),
                    timestamp: Some(0),
                });
    ...
    fn shake_compact_save_failure_does_not_claim_success() {
        ...
        let expected_entries = runtime().block_on(async {
            ...
            for i in 0..4 {
                session_guard.append_message(crate::session::SessionMessage::User {
                    content: pi_ai::model::UserContent::Text(format!("user-{i} {bulky}")),
                    timestamp: Some(i),
                });
    ...
    fn session_user(text: &str) -> crate::session::SessionMessage {
        crate::session::SessionMessage::User {
            content: pi_ai::model::UserContent::Text(text.to_string()),
            timestamp: Some(0),
        }
    }

    fn session_assistant(text: &str) -> crate::session::SessionMessage {
        crate::session::SessionMessage::from(pi_ai::model::Message::Assistant(Arc::new(
            AssistantMessage {
                content: vec![ContentBlock::Text(TextContent::new(text))],
                ..Default::default()
            },
    ...
    fn slash_retry_refuses_user_blocks_barrier_without_provider_calls() {
        ...
        runtime().block_on(async {
            ...
            session_guard.append_message(crate::session::SessionMessage::User {
                content: pi_ai::model::UserContent::Blocks(vec![ContentBlock::Text(
