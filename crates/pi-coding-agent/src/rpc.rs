...
pub struct RpcScopedModel {
    ...
    pub thinking_level: Option<pi_ai::model::ThinkingLevel>,
}
    ...
    match event {
        AgentEvent::MessageUpdate {
            assistant_message_event:
                pi_ai::model::AssistantMessageEvent::TextDelta { .. }
                | pi_ai::model::AssistantMessageEvent::ThinkingDelta { .. }
                | pi_ai::model::AssistantMessageEvent::ToolCallDelta { .. },
            ..
        } => RpcOutputPressureClass::MessageDelta,
...
struct RpcSharedState {
    ...
    failover_cooldown: Option<pi_ai::failover::CooldownTracker>,
    ...
    fn new_with_provider_admission(
        ...
        Self {
            ...
            failover_cooldown: config
                ...
                .map(|_| pi_ai::failover::CooldownTracker::new(config.failover_cooldown_secs())),
...
pub async fn run(
    ...
    while let Ok(line) = in_rx.recv(&cx).await {
        ...
        match command_type {
            ...
            "cycle_model" => {
                let result: Result<Option<(ModelEntry, pi_ai::model::ThinkingLevel, bool)>> =
                    async {
                        {
                            let mut guard = OwnedMutexGuard::lock(Arc::clone(&session), &cx)
                                .await
        ...
        *deferred_agent_end
        ...
        match result {
            Ok(message) => {
                if matches!(message.stop_reason, StopReason::Error | StopReason::Aborted) {
                    ...
                    if let Some(ref err_msg) = final_error {
                        ...
                        if !pi_error::is_retryable_error(
                            err_msg,
            ...
            Err(err) => {
                ...
                if !err.is_transient() && !pi_error::is_retryable_error(&err_str, None, None) {
                    final_error = Some(err_str);
                    final_error_hints = Some(error_hints_value(&err));
                    break;
                }
                ...
                final_error_hints = Some(error_hints_value(&err));
                if pi_ai::failover::classify_failover(&err_str)
                    == Some(pi_ai::failover::FailoverClass::Quota)
                {
...
async fn maybe_restore_primary(
    ...
    if !pi_ai::provider_metadata::provider_ids_match(runtime_provider.name(), &active_provider)
        || !runtime_provider
            .model_id()
    ...
    let session_matches_active = inner.effective_model_for_current_path().is_some_and(
        |(session_provider, session_model)| {
            pi_ai::provider_metadata::provider_ids_match(&session_provider, &active_provider)
...
async fn try_failover_to_next_chain_entry(
    ...
    let Some(class) = pi_ai::failover::classify_failover(error_text) else {
    ...
    let primary_model = match (
        ...
        (Some((active_provider, active_model)), Some(primary))
            if pi_ai::provider_metadata::provider_ids_match(&current_provider, active_provider)
    ...
    let Some(chain) = pi_ai::failover::chain_for(
        chains,
        "default",
    ...
    while position < chain.entries.len() {
        ...
        let is_current = pi_ai::provider_metadata::split_provider_model_spec(spec).is_some_and(
            |(provider, model_id)| {
                pi_ai::provider_metadata::provider_ids_match(&current_provider, provider)
...
mod retry_tests {
    ...
    use pi_ai::provider::{InputType, Model, ModelCost, Provider};
    use crate::resources::ResourceLoader;
    ...
    impl Provider for FlakyProvider {
        ...
        async fn stream(
            ...
            Pin<
                Box<
                    dyn futures::Stream<Item = pi_error::Result<pi_ai::model::StreamEvent>>
                        + Send,
                >,
            >,
        > {
            ...
            let events = if call == 0 {
                ...
                vec![
                    Ok(pi_ai::model::StreamEvent::Start {
                        partial: partial.clone(),
                    }),
                    Ok(pi_ai::model::StreamEvent::Error {
    ...
    fn rpc_multihop_failover_cooldown_restores_explicit_primary_identity() {
        ...
        runtime.block_on(async move {
            ...
            fallback.compat = Some(crate::models::CompatConfig {
                tool_call_dialect: Some(pi_ai::dialects::Dialect::Xmlish),
                ..Default::default()
            });
...
fn parse_prompt_images(value: Option<&Value>) -> Result<Vec<ImageContent>> {
    ...
    for item in arr {
        ...
        images.push(ImageContent {
            data: data.to_string(),
            mime_type: pi_ai::model::sanitize_image_mime_type(media_type),
        });
    }
    Ok(images)
}
...
fn parse_thinking_level(level: &str) -> Result<pi_ai::model::ThinkingLevel> {
    level.parse().map_err(|err: String| Error::validation(err))
}

fn session_thinking_level(
    session: &crate::session::Session,
) -> Option<pi_ai::model::ThinkingLevel> {
    session
        .effective_thinking_level_for_current_path()
        .as_deref()
        .and_then(|raw| {
...
fn normalize_resumed_session_model(
    ...
    entry: &ModelEntry,
) -> (pi_ai::model::ThinkingLevel, bool) {
...
fn extract_user_text(content: &pi_ai::model::UserContent) -> Option<String> {
    match content {
        pi_ai::model::UserContent::Text(text) => Some(text.clone()),
        pi_ai::model::UserContent::Blocks(blocks) => blocks.iter().find_map(|b| {
...
fn available_thinking_levels(entry: &ModelEntry) -> Vec<pi_ai::model::ThinkingLevel> {
...
async fn cycle_model_for_rpc(
    ...
    state: &mut RpcSharedState,
    options: &RpcOptions,
) -> Result<Option<(ModelEntry, pi_ai::model::ThinkingLevel, bool)>> {
    ...
    guard.agent.set_provider(provider_impl);
    guard.agent.set_keyword_max_thinking_level(
        next_entry.clamp_thinking_level(pi_ai::model::ThinkingLevel::Max),
    );
    guard
        .agent
...
mod tests {
    ...
    impl Provider for CompactionSummaryProvider {
        ...
        > {
            ...
            Ok(Box::pin(futures::stream::iter(vec![
                Ok(pi_ai::model::StreamEvent::TextDelta {
                    content_index: 0,
                    delta: summary.clone(),
                }),
                Ok(pi_ai::model::StreamEvent::TextEnd {
                    content_index: 0,
                    content: summary,
                }),
                Ok(pi_ai::model::StreamEvent::Done {
    ...
    impl Provider for GatedQueuedKeywordProvider {
        ...
        async fn stream(
            ...
            options: &pi_ai::provider::StreamOptions,
        ) -> pi_error::Result<
            Pin<
                Box<
                    dyn futures::Stream<Item = pi_error::Result<pi_ai::model::StreamEvent>>
...
"#;
    ...
    fn max_time_staged_input_survives_recorded_tool_transcript_recovery() {
        ...
        runtime.block_on(async {
            ...
            let completed_result = Message::ToolResult(Arc::new(pi_ai::model::ToolResultMessage {
    ...
    fn terminal_recovery_preserves_completed_tool_effects_after_task_drop() {
        ...
        runtime.block_on(async {
            ...
            let tool_result = Message::ToolResult(Arc::new(pi_ai::model::ToolResultMessage {
                tool_call_id: "tool-1".to_string(),
                tool_name: "write".to_string(),
    ...
    fn terminal_recovery_refuses_an_incomplete_multi_tool_cycle() {
        ...
        let only_first_result = Message::ToolResult(Arc::new(pi_ai::model::ToolResultMessage {
    ...
    fn terminal_recovery_accepts_a_fully_completed_multi_tool_cycle() {
        ...
        let first_result = Message::ToolResult(Arc::new(pi_ai::model::ToolResultMessage {
            tool_call_id: "tool-a".to_string(),
            tool_name: "write".to_string(),
            content: vec![ContentBlock::Text(TextContent::new("wrote a.txt"))],
            details: None,
        ...
        let second_result = Message::ToolResult(Arc::new(pi_ai::model::ToolResultMessage {
    ...
    fn terminal_recovery_ignores_pause_turn_server_calls_before_local_tool_effects() {
        ...
        let local_result = Message::ToolResult(Arc::new(pi_ai::model::ToolResultMessage {
