...
use pi_ai::sse::{SseEvent, SseStream};
...
impl OpenAIResponsesProvider {
    ...
    pub fn build_request(
        ...
        let (tool_choice, parallel_tool_calls, text, include, reasoning) = if self.codex_mode {
            ...
            .thinking_level
            .filter(|level| self.reasoning && *level != pi_ai::model::ThinkingLevel::Off)
        {
    ...
    async fn stream(
        ...
        if let Some(hook) = &options.before_provider_request {
            match serde_json::to_value(&request_body) {
                Ok(original) => {
                    let event = pi_ai::provider::BeforeProviderRequestEvent {
...
mod tests {
    ...
    fn test_build_request_thinking_level_map_overrides_effort() {
        ...
        let medium = provider.build_request(
            ...
            &StreamOptions {
                thinking_level: Some(pi_ai::model::ThinkingLevel::Medium),
                ..Default::default()
            },
        );
    ...
    fn test_build_request_omits_reasoning_when_disabled_or_off() {
        let cases = [
            (false, Some(pi_ai::model::ThinkingLevel::High)),
            (true, Some(pi_ai::model::ThinkingLevel::Off)),
            (true, None),
        ];
    ...
    fn tool_result_message(tool_call_id: &str, text: &str) -> Message {
        Message::ToolResult(std::sync::Arc::new(pi_ai::model::ToolResultMessage {
    ...
    fn test_stream_errors_on_malformed_function_call_done_item() {
        ...
        let event_source = pi_ai::sse::SseStream::new(Box::pin(empty));
...
pub mod fuzz {
    ...
    type FuzzStream = pi_ai::sse::SseStream<FuzzByteStream>;
