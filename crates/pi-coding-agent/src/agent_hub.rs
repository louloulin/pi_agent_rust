...
//! through a steering `MessageFetcher` between turns. The parent's
//! in-memory registry is the roster of record; the on-disk queue files are
...
/// Resolve the global directory for hub artifacts.
///
/// Mirrors `crate::config::Config::global_dir()`: honors `PI_CODING_AGENT_DIR`,
/// otherwise returns `<home>/.pi/agent`. Inlined here so this crate does
/// not have to depend on the in-`pi` config module.
fn global_dir() -> PathBuf {
    if let Some(override_path) = std::env::var_os("PI_CODING_AGENT_DIR") {
...
impl AgentHubRegistry {
    ...
    pub fn transcript_page(&self, id: &str) -> Result<String> {
        ...
        let (masked, _audit) = obfuscate(&tail, &mut vault, &[]);
        Ok(masked)
    }
