//! Shared platform identity and filesystem-access policy utilities.
//!
//! This crate provides the platform naming conventions used in provider
//! requests and the deterministic Unix permission checks shared by session,
//! model, tool, and extension boundaries.
...
#[derive(Debug, Clone)]
pub struct EffectiveModeAccessContext {
    #[cfg(unix)]
...
#[cfg(unix)]
pub fn read_trusted_symlink_component(
...
pub fn ensure_effective_mode_access(
    metadata: &std::fs::Metadata,
    path: &Path,
    required: u32,
    operation: &str,
