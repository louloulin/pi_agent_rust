...
use pi_ai::model::{ContentBlock, StreamEvent, TextContent};
...
fn test_model_entry() -> ModelEntry {
    ModelEntry {
        model: pi_ai::provider::Model {
            ...
            reasoning: true,
            input: vec![pi_ai::provider::InputType::Text],
...
fn new_session_thinking_level_resolves_configured_default() {
    ...
    assert_eq!(
        app.new_session_thinking_level(),
        app.model_entry
            .clamp_thinking_level(pi_ai::model::ThinkingLevel::XHigh)
    );
}
