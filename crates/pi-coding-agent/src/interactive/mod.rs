...
impl PiApp {
    ...
    fn persist_project_theme(&self, theme_name: &str) -> pi_error::Result<()> {
...
impl PiApp {
    ...
    pub fn set_btw_client(&mut self, client: Arc<crate::btw::BtwClient>) {
        self.btw_client = Some(client);
    }

    /// Install the factory used to rebind the `/btw` client on smol-role
    /// change (bd-9jgrt).
    pub fn set_btw_factory(&mut self, factory: crate::btw::BtwClientFactory) {
    ...
    pub fn new(
        ...
        let startup_changelog = prepare_startup_changelog_with_roots(
            ...
            VERSION,
            pi_ai::embedded_assets::changelog,
        );
