...
//! are deterministically compressed by the `pi-embedded-assets` build script;
//! inline JavaScript literals use the const LZSS codec below. This crate
//! decodes both forms on demand. Parse-only callers own a temporary
//! `String`; repeatedly used resources are retained by their owning
//! `OnceLock`. Every caller receives the exact original UTF-8 bytes.
