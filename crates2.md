...
**做了什么:**
1. 新增 `crates/pi-current-time-core`，承载 `TimeSnapshot` 的 Now、Duration/epoch 字段与格式化/详情纯逻辑。
2. `current_time` tool runtime 保留在主 crate，仅依赖并调用时间核心 crate。
3. workspace 注册核心 crate，并保留原有工具契约与测试覆盖。

**验证:**
- `cargo test -p pi-current-time-core` ✅
- `git diff --check` ✅
