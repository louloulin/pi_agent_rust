...
fn build_scanned_meta(path: &Path) -> pi_error::Result<SessionMeta> {
    #[cfg(test)]
...
mod tests {
    ...
    fn build_meta_from_file_rejects_semantically_invalid_header() {
        ...
        assert!(
            matches!(err, pi_error::Error::Session(ref msg) if msg.contains("Invalid session header")),
