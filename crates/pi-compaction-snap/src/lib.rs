...
use pi_model::{ContentBlock, ImageContent, Message, TextContent, UserContent};
...
/// Inner padding (in scaled pixels) between frame edge and content.
const PAD: u32 = 8;
/// Frame width in pixels (96 columns × 10px advance).
const FRAME_WIDTH: u32 = 960;
/// Maximum frame height in pixels.
const MAX_FRAME_HEIGHT: u32 = 1280;
/// Terminal columns per frame, derived from geometry.
const CHARS_PER_LINE: u32 = (FRAME_WIDTH - 2 * PAD) / CHAR_ADVANCE;
/// Lines per frame, derived from geometry.
const LINES_PER_FRAME: u32 = (MAX_FRAME_HEIGHT - 2 * PAD) / LINE_HEIGHT;

type Rgb = [u8; 3];

/// 5×7 ASCII font (chars 0x20–0x7E), one byte per column, LSB = top row.
///
/// This is the classic public-domain `glcdfont` table, identical bytes to
/// the Adafruit_GFX redistribution. Each row is 7 bits (top row = bit 0).
...
/// Pick a foreground color for a transcript line based on its label
/// (User / Assistant / Tool / headers), matching `serialize_conversation`'s
/// markdown headers.
