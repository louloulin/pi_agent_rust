...
/// Input types a model can accept, as declared by the provider catalog.
///
/// This is the single source of truth: `pi::provider` re-exports it rather
/// than defining its own copy, so the catalog constants below and the runtime
/// `Model::input` vectors are always the same type.
            ...
            match meta.onboarding {
                ProviderOnboardingMode::OpenAICompatiblePreset => {
                    ...
                    if meta.auth_env_keys.is_empty() {
                        ...
                        assert!(
                            provider_is_keyless_local(meta.canonical_id),
