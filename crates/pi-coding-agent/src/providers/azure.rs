    ...
    S: Stream<Item = std::result::Result<Vec<u8>, std::io::Error>> + Unpin,
{
    ...
    fn push_text_delta(&mut self, text: String) -> StreamEvent {
        ...
        if !last_is_text {
            ...
            self.partial
                .content
                .push(ContentBlock::Text(pi_ai::model::TextContent::new("")));
            self.pending_events
            ...
            if let Some(tool_calls) = choice.delta.tool_calls {
                ...
                for tc in tool_calls {
                        ...
                        self.tool_calls.iter().position(|tc| tc.index == idx)
                    {
                        ...
                        self.partial
                            .content
                            .push(ContentBlock::ToolCall(pi_ai::model::ToolCall {
                                id: String::new(),
                                name: String::new(),
...
mod tests {
    use super::*;
    use pi_ai::model::{ImageContent, TextContent, ToolCall, ToolResultMessage, UserMessage};
