...
/// HTTP capability accessor was removed during Stage 2 Batch 17 extraction:
/// the previous `AgentHttp::client()` returned `crate::http::client::Client`,
/// a hard cross-module dep on the not-yet-extracted `http` module. No caller
/// in `crates/pi/src/` invokes `AgentHttp::client()` (only `cx.http()` is
/// referenced in a self-test), so the capability is dropped here and will
/// return as a leaf of its own when `pi-http` lands.
