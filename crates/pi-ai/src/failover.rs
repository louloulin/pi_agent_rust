...
/// prefixes and the global settings. Owned here because failover is the only
/// consumer; `pi::config` re-exports it so settings parsing stays unchanged.
...
pub fn classify_failover(error_text: &str) -> Option<FailoverClass> {
    ...
    if pi_error::is_retryable_error(&text.to_ascii_lowercase(), None, None) {
...
impl CooldownTracker {
    ...
    /// Deterministic view of the failure timestamp.
    ///
    /// No longer `#[cfg(test)]`: the leaf crate is compiled without `cfg(test)`
    /// when the `pi` crate's test binary consumes it, so the accessor must
    /// exist unconditionally for `pi`'s failover-state assertions.
    #[must_use]
...
impl CredentialRing {
    ...
    #[must_use]
    pub fn key_fingerprints(&self) -> Vec<String> {
        self.keys
            .iter()
            .map(|key| {
                let len = key.len();
...
pub fn best_scope_override<'a>(
    overrides: &'a [ModelScopeOverride],
    cwd: &std::path::Path,
) -> Option<&'a ModelScopeOverride> {
    overrides
        .iter()
        .filter(|ov| {
...
pub fn provider_is_disabled(
    disabled: &[String],
    scope: Option<&ModelScopeOverride>,
    provider: &str,
) -> bool {
