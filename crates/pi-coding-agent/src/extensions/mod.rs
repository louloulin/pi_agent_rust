...
pub use crate::extensions_api::{
    COMPAT_LEDGER_SCHEMA_VERSION, CompatCapabilityEvidence, CompatLedger, CompatibilityScanner,
    DangerousCommandClass, DangerousOptInAuditEntry, Error, EXTENSION_EVENT_TIMEOUT_MS,
    EXTENSION_SHORTCUT_BUDGET_MS, EventCoalescer, ExtensionAiCompletionRequest,
    ExtensionDeliverAs, ExtensionEventName, ExtensionHostActions, ExtensionLoadSpec,
    ExtensionManager, ExtensionMessage, ExtensionOverride, ExtensionPolicy, ExtensionPolicyMode,
    ExtensionRegion, ExtensionRepairEvent, ExtensionRuntimeHandle, ExtensionSendMessage,
    ExtensionSendUserMessage, ExtensionSession, ExtensionToolDef, ExtensionUiRequest,
    ExtensionUiResponse, HostCallError, HostCallErrorCode, HostCallPayload, HostResultPayload,
    HostStreamBackpressure, HostStreamChunk, JsExtensionLoadSpec, JsExtensionRuntimeHandle,
    JsExtensionSnapshot, NativeRustExtensionLoadSpec, NativeRustExtensionRuntimeHandle,
    PROTOCOL_VERSION, PolicyCheck, PolicyDecision, PolicyProfile, PolicySnapshot, RepairPolicyMode,
    Result, RuntimeRiskConfig, SecretBrokerPolicy, SessionActionOrigin, ToolResultPayload,
    WasmExtensionHandle, apply_cli_flags, classify_dangerous_command, coerce_cli_flag_value,
    evaluate_exec_mediation, extract_slash_command_name, load_extension_manifest,
    parse_extension_tool_defs, resolve_extension_load_spec, safe_canonicalize, strip_unc_prefix,
};
