...
fn tool_err(code: &str, message: impl Into<String>) -> pi_error::Error {
    pi_error::Error::tool("debug", format!("[{code}] {}", message.into()))
}
...
impl DapSession {
    ...
    pub async fn begin(transport: DapTransport) -> pi_error::Result<Self> {
        let session = Self {
            transport,
    ...
    fn require_stopped(&self) -> pi_error::Result<u64> {
        match self.state() {
    ...
    pub async fn call(&self, command: &str, arguments: Value) -> pi_error::Result<Value> {
        self.transport
