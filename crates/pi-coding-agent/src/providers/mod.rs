...
impl ExtensionStreamSimpleProvider {
    ...
    const fn new(model: pi_ai::provider::Model, runtime: ExtensionRuntimeHandle) -> Self {
        Self { model, runtime }
    }

    fn build_js_model(model: &pi_ai::provider::Model) -> Value {
        serde_json::json!({
            "id": &model.id,
            "name": &model.name,
            "api": &model.api,
    ...
    fn build_js_options(options: &StreamOptions) -> Value {
        ...
        let cache_retention = match options.cache_retention {
            pi_ai::provider::CacheRetention::None => "none",
            pi_ai::provider::CacheRetention::Short => "short",
            pi_ai::provider::CacheRetention::Long => "long",
        };
        map.insert(
        ...
        if let Some(level) = options.thinking_level
            && level != pi_ai::model::ThinkingLevel::Off
        {
        ...
        match event {
            ...
            AssistantMessageEvent::ToolCallStart {
                ...
                let (id, name) = match partial.content.get(content_index) {
                    Some(pi_ai::model::ContentBlock::ToolCall(tc)) => {
