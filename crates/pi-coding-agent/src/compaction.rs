...
fn media_token_estimate(media: &pi_ai::model::MediaContent) -> u64 {
...
fn estimate_tokens(message: &SessionMessage) -> u64 {
    ...
    let table = assistant_provider.map_or(
        pi_ai::token_count::TokenTable::O200k,
        pi_ai::token_count::table_for_provider,
    );
    flat_tokens.saturating_add(pi_ai::token_count::active_counter().count(&text, table))
}
...
async fn complete_simple(
    ...
    while let Some(event) = stream.next().await {
        match event? {
            pi_ai::model::StreamEvent::Done { message, .. } => {
                final_message = Some(message);
            }
            pi_ai::model::StreamEvent::Error { error, .. } => {
                let msg = error
                    .error_message
...
fn finish_compaction(preparation: CompactionPreparation, mut summary: String) -> CompactionResult {
    ...
    let snap_payload = if matches!(
        ...
        let model_messages: Vec<pi_ai::model::Message> = preparation
            .messages_to_summarize
            .iter()
            .filter_map(session_message_to_model)
            .collect();
...
mod tests {
    ...
    use super::*;
    use pi_ai::model::{AssistantMessage, ContentBlock, TextContent, Usage};
    use serde_json::json;
    ...
    use pi_ai::model::{ImageContent, ThinkingContent};
    use crate::session::{
    ...
    fn serialize_conversation_tool_result() {
        let messages = vec![Message::tool_result(pi_ai::model::ToolResultMessage {
    ...
    fn compact_auto_shake_first_skips_llm_when_reclaim_suffices() {
        ...
        impl pi_ai::provider::Provider for PanickingProvider {
            fn name(&self) -> &str {
                "panic-provider"
            }
            fn api(&self) -> &str {
            ...
            async fn stream(
                ...
                std::pin::Pin<
                    Box<
                        dyn futures::Stream<
                                Item = pi_error::Result<pi_ai::provider::StreamEvent>,
                            > + Send,
                    >,
                >,
            > {
    ...
    mod fallback {
        ...
        impl Provider for FailingProvider {
            ...
            async fn stream(
                ...
                Pin<Box<dyn Stream<Item = pi_error::Result<pi_ai::model::StreamEvent>> + Send>>,
            > {
