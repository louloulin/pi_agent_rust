...
### Round 78 — `pi-path-core` 路径工具核心拆分

**做了什么:**
1. 新增 `pi-path-core`，承载纯词法 path normalization、relative/absolute resolution，以及 `OsStr`/`OsString` 安全转换；不触碰文件系统，保留 prefix/root/非 UTF-8 组件。
2. `pi`/coding-agent 的 tools 与 package manager 继续保留历史 facade，改为调用 `pi_path_core::normalize`，agent runtime 与 UI 未迁移。
3. 为 absolute/relative、`.`/`..`、UTF-8/非 UTF-8 路径补充核心 crate 单元测试。

**验证:**
- `git diff --check` ✅
- `cargo test -p pi-path-core`：当前运行环境未提供 `cargo`，无法执行
- `cargo check -p pi`：当前运行环境未提供 `cargo`，无法执行

**进度:** Round 78 path 核心完成；本地工作树已准备 commit/push。
