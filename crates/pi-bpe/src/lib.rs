#[rustfmt::skip]
/// This file is a vendored copy of the `tiktoken` crate (MIT), via
/// tiktoken-rs 0.12.0's `vendor_tiktoken.rs` (itself a tiktoken vendoring).
/// Vendored into pi (bd-w8q6u) so the BPE rank tables load from our
/// gzip-compressed assets instead of tiktoken-rs's `include_str!` + lazy_static
/// embedding, which rooted ~5.3 MiB of raw table text in the release binary.
/// Added beyond tiktoken-rs's copy: the `CoreBPE::new` constructor (from
/// tiktoken-rs's `patched_tiktoken.rs`, same license).
...
pub type BpeResult<T> = std::result::Result<T, BpeError>;
...
impl CoreBPE {
    /// Constructor (from tiktoken-rs 0.12.0 `patched_tiktoken.rs`, MIT):
    /// builds the merge-rank core with its split + special-token regexes.
    pub fn new(
        encoder: HashMap<Vec<u8>, Rank>,
        special_tokens_encoder: HashMap<String, Rank>,
        pattern: &str,
    ) -> crate::BpeResult<Self> {
        let regex = Regex::new(pattern)
            .map_err(|e| crate::BpeError::validation(format!("tokenizer pattern: {e}")))?;
        ...
        let special_regex = {
            let parts = special_tokens_encoder
                ...
                .collect::<Vec<_>>();
            Regex::new(&parts.join("|")).map_err(|e| {
                crate::BpeError::validation(format!("tokenizer special regex: {e}"))
            })?
        };
        ...
        if encoder.len() != decoder.len() {
            return Err(crate::BpeError::validation(
                "encoder has duplicate token indices",
            ));
        }
