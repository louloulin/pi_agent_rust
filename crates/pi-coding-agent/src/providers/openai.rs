    ...
    async fn stream(
        ...
        let auth_value = if authorization_override.is_some() || !self.auth_header {
            ...
            match resolved {
                ...
                None if pi_ai::provider_metadata::provider_is_keyless_local(self.name()) => None,
...
mod tests {
    ...
    fn test_convert_media_block_degrades_to_placeholder() {
        let media = pi_ai::model::MediaContent {
            data: "AAAA".to_string(),
        ...
        let result = Message::tool_result(pi_ai::model::ToolResultMessage {
    ...
    fn test_convert_user_text_message() {
        let message = Message::User(pi_ai::model::UserMessage {
    ...
    fn test_build_request_prompt_cache_key_serialize_and_omit() {
        ...
        let context = Context {
            system_prompt: None,
            messages: vec![Message::User(pi_ai::model::UserMessage {
    ...
    fn test_build_request_deepseek_forwards_thinking_and_reasoning_effort() {
        ...
        let body = |provider: &OpenAIProvider, level: pi_ai::model::ThinkingLevel| {
            let context = Context {
                system_prompt: None,
                messages: vec![Message::User(pi_ai::model::UserMessage {
        ...
        let off = body(&ds, pi_ai::model::ThinkingLevel::Off);
        ...
        let high = body(&ds, pi_ai::model::ThinkingLevel::High);
        assert_eq!(high["thinking"]["type"], "enabled");
        assert_eq!(high["reasoning_effort"], "high");

        let xhigh = body(&ds, pi_ai::model::ThinkingLevel::XHigh);
        ...
        let medium = body(&ds, pi_ai::model::ThinkingLevel::Medium);
        ...
        let ds_by_url = OpenAIProvider::new("deepseek-v4-pro")
            ...
            .with_reasoning(true);
        let high_url = body(&ds_by_url, pi_ai::model::ThinkingLevel::High);
    ...
    fn test_build_request_compat_thinking_format_deepseek_custom_provider() {
        ...
        let off = body(&custom, pi_ai::model::ThinkingLevel::Off);
        assert_eq!(off["thinking"]["type"], "disabled");
        assert!(
            off.get("reasoning_effort").is_none(),
            "off must not send reasoning_effort"
        ...
        let high = body(&custom, pi_ai::model::ThinkingLevel::High);
        assert_eq!(high["thinking"]["type"], "enabled");
        assert_eq!(high["reasoning_effort"], "high");

        let xhigh = body(&custom, pi_ai::model::ThinkingLevel::XHigh);
        assert_eq!(xhigh["thinking"]["type"], "enabled");
        assert_eq!(xhigh["reasoning_effort"], "max");

        let max = body(&custom, pi_ai::model::ThinkingLevel::Max);
        ...
        let quiet = body(&non_reasoning, pi_ai::model::ThinkingLevel::High);
    ...
    fn test_build_request_thinking_level_map_overrides_deepseek_effort() {
        ...
        let body = |level: pi_ai::model::ThinkingLevel| {
            let context = Context {
                system_prompt: None,
                messages: vec![Message::User(pi_ai::model::UserMessage {
                    content: UserContent::Text("Solve it".to_string()),
        ...
        let xhigh = body(pi_ai::model::ThinkingLevel::XHigh);
        assert_eq!(xhigh["thinking"]["type"], "enabled");
        assert_eq!(xhigh["reasoning_effort"], "high", "map overrides max");

        let medium = body(pi_ai::model::ThinkingLevel::Medium);
        assert_eq!(
            medium["reasoning_effort"], "medium",
            "map can emit an effort for a level with no default"
        );

        let max = body(pi_ai::model::ThinkingLevel::Max);
        assert_eq!(
            max["reasoning_effort"], "max",
            "unmapped level keeps default"
        );

        let high = body(pi_ai::model::ThinkingLevel::High);
        assert_eq!(high["reasoning_effort"], "high");

        let off = body(pi_ai::model::ThinkingLevel::Off);
        assert_eq!(off["thinking"]["type"], "disabled");
        assert!(
            off.get("reasoning_effort").is_none(),
    ...
    fn test_build_request_compat_thinking_format_explicit_declaration_wins() {
        ...
        let options = StreamOptions {
            thinking_level: Some(pi_ai::model::ThinkingLevel::High),
            ..Default::default()
        };
        let value = serde_json::to_value(provider.build_request(&context, &options))
            .expect("serialize request");
    ...
    fn test_build_request_non_reasoning_deepseek_omits_thinking() {
        ...
        let options = StreamOptions {
            thinking_level: Some(pi_ai::model::ThinkingLevel::Off),
            ..Default::default()
        };
        let value = serde_json::to_value(provider.build_request(&context, &options))
            .expect("serialize request");
    ...
    fn test_build_request_deepseek_provider_alias_detected() {
        ...
        let options = StreamOptions {
            thinking_level: Some(pi_ai::model::ThinkingLevel::XHigh),
            ..Default::default()
        };
        let value = serde_json::to_value(provider.build_request(&context, &options))
            .expect("serialize request");
    ...
    fn openrouter_body(provider: &OpenAIProvider, level: pi_ai::model::ThinkingLevel) -> Value {
        let context = Context {
            system_prompt: None,
            messages: vec![Message::User(pi_ai::model::UserMessage {
                content: UserContent::Text("Solve it".to_string()),
    ...
    fn stream_rejects_transport_eof_before_done_sentinel() {
        ...
        assert!(pi_error::is_retryable_error(&error, None, None));
    }
    ...
    fn collect_events(events: &[Value]) -> Vec<StreamEvent> {
        ...
        runtime.block_on(async move {
            ...
            let event_source = pi_ai::sse::SseStream::new(Box::pin(byte_stream));
    ...
    mod proptest_process_event {
        ...
        -> StreamState<impl Stream<Item = std::result::Result<Vec<u8>, std::io::Error>> + Unpin>
        {
            ...
            let sse = pi_ai::sse::SseStream::new(Box::pin(empty));
    ...
    impl Processor {
        ...
        pub fn new() -> Self {
            ...
            Self(StreamState::new(
                pi_ai::sse::SseStream::new(Box::pin(empty)),
                "gpt-fuzz".into(),
                "openai".into(),
                "openai".into(),
            ))
        ...
        pub fn process_event(&mut self, data: &str) -> pi_error::Result<Vec<StreamEvent>> {
