...
fn plan_error(code: &str, message: impl Into<String>) -> pi_error::Error {
    pi_error::Error::tool("lsp", format!("[{code}] {}", message.into()))
}
...
pub fn parse_workspace_edit(raw: &Value) -> Result<WorkspaceEditPlan, pi_error::Error> {
...
fn parse_text_edit_array(raw: &Value) -> Result<Vec<TextEdit>, pi_error::Error> {
...
fn validate_file_ops(ops: &[FileOp]) -> Result<(), pi_error::Error> {
    for op in ops {
        match op {
...
pub fn apply_workspace_edit(
...
) -> Result<ApplyOutcome, pi_error::Error> {
    ...
    let commit_result: Result<(), pi_error::Error> = (|| {
