...
//! - Translating model `Message` history into provider-specific HTTP requests.
//! - Emitting [`StreamEvent`] values as SSE/HTTP chunks arrive.
//! - Advertising tool schemas to the model (so it can call host tools by name).
...
pub trait Provider: Send + Sync {
    ...
    ) -> pi_error::Result<Pin<Box<dyn Stream<Item = pi_error::Result<StreamEvent>> + Send>>>;
    ...
    ) -> pi_error::Result<serde_json::Value> {
