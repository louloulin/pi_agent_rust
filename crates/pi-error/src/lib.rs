...
use pi_provider_metadata::{canonical_provider_id, provider_auth_env_keys};
...
mod tests {
    ...
    fn e2e_alias_env_key_consistency() {
        ...
        for (alias, canonical) in alias_to_canonical {
            let alias_keys = pi_provider_metadata::provider_auth_env_keys(alias);
            let canonical_keys = pi_provider_metadata::provider_auth_env_keys(canonical);
            assert_eq!(
    ...
    fn e2e_all_native_providers_have_env_keys() {
        ...
        for provider in &native_providers {
            let keys = pi_provider_metadata::provider_auth_env_keys(provider);
